#!/usr/bin/env python3
"""
CPU-Only Object Detection using Feature Matching (Baseline for comparison)
Detects balls and books in static images using standard OpenCV (no CUDA)
...
class CPUObjectDetector:
    """
    CPU-only object detector using ORB feature matching and homography.
    Baseline implementation for performance comparison with GPU version.
    """
    
    def __init__(self, template_dir: str, min_matches: int = 10, ratio_threshold: float = 0.75):
        """
        Initialize CPU object detector.
        ...
        # Create CPU Brute Force Matcher
        self.matcher = cv2.BFMatcher(cv2.NORM_HAMMING, crossCheck=False)
        
        print("[INFO] CPU-only mode initialized")
    ...
    def _load_templates(self):
        ...
        for obj_name, filename in template_files.items():
            ...
            print(f"[INFO] Loaded template '{obj_name}': {len(keypoints)} keypoints, "
                  f"size {w}x{h}, extraction time {extract_time*1000:.2f} ms")
    ...
    def detect(self, image: np.ndarray) -> List[Dict]:
        ...
        for obj_name, template in self.templates.items():
            ...
            # CPU k-NN matching (k=2 for ratio test)
...
def main():
    """Main function for testing CPU object detector."""
    import argparse
    
    parser = argparse.ArgumentParser(description='CPU Object Detection using Feature Matching (Baseline)')
    parser.add_argument('--templates', type=str, default='../templates',
                        help='Directory containing template images')
    parser.add_argument('--input', type=str, required=True,
                        help='Input image path or directory')
    parser.add_argument('--output', type=str, default='../results/cpu',
    ...
    # Initialize detector
    print("="*60)
    print("CPU Object Detector - Feature Matching (Baseline)")
    print("="*60)
    ...
    if os.path.isfile(args.input):
        ...
        output_path = os.path.join(
            args.output,
            f"cpu_result_{os.path.basename(args.input)}"
        )
        ...
        for img_file in image_files:
            ...
            output_path = os.path.join(args.output, f"cpu_result_{img_file}")
        ...
        # Summary
        print("\n" + "="*60)
        print("BATCH PROCESSING SUMMARY (CPU)")
        print("="*60)
